...
ENTRIES = [
  ...
  # ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    'formula_latex': r'\\text{Total Bilan} = \\sum \\text{Actifs Bilanciels}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{Dépôts} = \\sum \\text{Ressources Clientèle}',
    'interpretation': {
    ...
    'related_terms': ['total-bilan', 'credits-clientele', 'ratio-prets-depots', 'depots-total-passif'],
    'display_order': 2
  },
  {
    'id': 'credits-clientele',
    'slug': 'credits-clientele',
    'term': {'fr': 'Crédits à la clientèle', 'en': 'Customer Loans', 'ar': 'القروض الممنوحة للعملاء'},
    'acronym': 'CRE',
    'aliases': {
      'fr': ['Prêts à la clientèle', 'Encours de crédits', 'Financements accordés'],
      'en': ['Customer loans', 'Gross loans', 'Credit portfolio'],
      'ar': ['التمويلات الممنوحة', 'حجم القروض', 'محفظة الائتمان']
    },
    'category': 'financial_indicators',
    'short_definition': {
    ...
    'formula_latex': r'\\text{Crédits} = \\sum \\text{Engagements Clientèle}',
    'interpretation': {
    ...
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Classements', 'Profil Banque', 'Historique', 'Parts de marché', 'Comparateur'],
    'related_terms': ['total-bilan', 'depots-clientele', 'ratio-prets-depots', 'prets-total-actif', 'taux-npl'],
    'display_order': 3
  },
  {
    'id': 'fonds-propres',
    'slug': 'fonds-propres',
    'term': {'fr': 'Fonds Propres Comptables', 'en': 'Accounting Equity', 'ar': 'حقوق الملكية (الأموال الخاصة)'},
    'acronym': 'FP',
    'aliases': {
      'fr': ['Capitaux propres', 'Fonds propres bilanciels', 'Net comptable'],
      'en': ['Total Equity', 'Book Value', 'Shareholders Equity'],
      'ar': ['الأموال الذاتية', 'صافي الأصول المحاسبية']
    },
    'category': 'financial_indicators',
    'short_definition': {
    ...
    'formula_latex': r'\\text{Fonds Propres} = \\text{Capital} + \\text{Réserves} + \\text{Report} + \\text{Résultat Net}',
    'interpretation': {
    ...
    'abix_benchmark': None,
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Classements', 'Scores ABIX (Capitalisation)'],
    'related_terms': ['total-bilan', 'return-on-equity', 'fonds-propres-total-bilan', 'ratio-solvabilite'],
    'display_order': 4
  },
  {
    'id': 'produit-net-bancaire',
    'slug': 'produit-net-bancaire',
    'term': {'fr': 'Produit Net Bancaire', 'en': 'Net Banking Income', 'ar': 'الناتج البنكي الصافي'},
    'acronym': 'PNB',
    'aliases': {
      'fr': ['Revenu bancaire net', 'Chiffre d\'affaires bancaire', 'NBI'],
      'en': ['Net Banking Income', 'NBI', 'Banking Operating Revenue'],
      'ar': ['الدخل المصرفي الصافي', 'إيرادات النشاط المصرفي']
    },
    'category': 'financial_indicators',
    'short_definition': {
    ...
    'formula_latex': r'\\text{PNB} = \\text{Marge d\'intérêt} + \\text{Commissions nettes} + \\text{Résultat financier & change}',
    'interpretation': {
    ...
    'abix_benchmark': None,
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Classements', 'Scores ABIX (Dynamique)', 'Historique'],
    'related_terms': ['resultat-brut-exploitation', 'resultat-net', 'coefficient-exploitation', 'marge-interet-nette'],
    'display_order': 5
  },
  {
    'id': 'resultat-brut-exploitation',
    'slug': 'resultat-brut-exploitation',
    'term': {'fr': 'Résultat Brut d\'Exploitation', 'en': 'Gross Operating Income', 'ar': 'النتيجة الإجمالية للاستغلال'},
    'acronym': 'RBE',
    'aliases': {
      'fr': ['Marge opérationnelle brute', 'GOI'],
      'en': ['Gross Operating Profit', 'GOI', 'Operating Profit before provisions'],
      'ar': ['الربح التشغيلي الإجمالي', 'فائض الاستغلال']
    },
    'category': 'financial_indicators',
    'short_definition': {
    ...
    'formula_latex': r'\\text{RBE} = \\text{PNB} - \\text{Frais généraux} - \\text{Amortissements}',
    'interpretation': {
    ...
    'abix_benchmark': None,
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Classements', 'Scores ABIX (Efficience)'],
    'related_terms': ['produit-net-bancaire', 'resultat-net', 'coefficient-exploitation', 'conversion-rbe-rn'],
    'display_order': 6
  },
  {
    'id': 'resultat-net',
    'slug': 'resultat-net',
    'term': {'fr': 'Résultat Net de l\'exercice', 'en': 'Net Profit (Net Income)', 'ar': 'النتيجة الصافية للسنة المالية'},
    'acronym': 'RN',
    'aliases': {
      'fr': ['Bénéfice net', 'Résultat net comptable', 'Net Income'],
      'en': ['Net Income', 'Net Profit', 'Bottom Line'],
      'ar': ['صافي الأرباح', 'النتيجة الصافية']
    },
    'category': 'financial_indicators',
    'short_definition': {
    ...
    'formula_latex': r'\\text{RN} = \\text{RBE} - \\text{Coût du risque} \\pm \\text{Exceptionnel} - \\text{Impôts}',
    'interpretation': {
    ...
    'abix_benchmark': None,
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Classements', 'Scores ABIX (Rentabilité)', 'Historique'],
    'related_terms': ['produit-net-bancaire', 'resultat-brut-exploitation', 'return-on-equity', 'return-on-assets'],
    'display_order': 7
  },
  {
    'id': 'bons-du-tresor',
    'slug': 'bons-du-tresor',
    'term': {'fr': 'Bons du Trésor & Titres d\'État', 'en': 'Treasury Securities & Sovereign Bonds', 'ar': 'سندات الخزينة والأوراق السيادية'},
    'acronym': 'BDT',
    'aliases': {
      'fr': ['Titres souverains', 'Placements en Bons du Trésor', 'Obligations du Trésor'],
      'en': ['Treasury Bonds', 'Sovereign Debt', 'Government Securities'],
      'ar': ['السندات السيادية', 'سندات الخزينة العمومية']
    },
    'category': 'financial_indicators',
    'short_definition': {
    ...
    'formula_latex': r'\\text{BDT} = \\sum \\text{Titres d\'État}',
    'interpretation': {
    ...
    'abix_benchmark': None,
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Classements', 'Historique', 'Allocation d\'actifs'],
    'related_terms': ['total-bilan', 'credits-clientele', 'bdt-total-actif'],
    'display_order': 8
  },
  ...
  # ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    'term': {'fr': 'Return on Equity (Rentabilité des Fonds Propres)', 'en': 'Return on Equity (ROE)', 'ar': 'العائد على حقوق الملكية'},
    'acronym': 'ROE',
    'aliases': {
      'fr': ['Rentabilité financière', 'Rendement des capitaux propres', 'ROE de clôture'],
      'en': ['ROE', 'Return on Equity', 'Shareholders Return'],
      'ar': ['مردودية الأموال الخاصة', 'العائد المالي للمساهمين']
    },
    'category': 'ratios_profitability',
    'short_definition': {
      'fr': 'Rapport entre le résultat net et les fonds propres de clôture, mesurant le rendement généré pour les actionnaires.',
      'en': 'Ratio of net income to closing equity, measuring financial return generated for shareholders.',
      'ar': 'النسبة بين النتيجة الصافية وحقوق الملكية في ختام الدورة، وتقيس العائد المالي المحقق للمساهمين.'
    },
    'detailed_definition': {
      'fr': 'Dans ABIX Data Explorer, le ROE standard est calculé en rapportant le résultat net aux fonds propres comptables de clôture au 31 décembre. Il mesure la capacité de la banque à rémunérer les capitaux investis. Note méthodologique : pour l\'analyse dynamique pluriannuelle, se référer également au ROAE calculé sur fonds propres moyens.',
      'en': 'In ABIX Data Explorer, standard ROE is calculated by dividing net profit by closing book equity as of December 31. It measures the return generated on invested capital. Methodology note: for multi-year dynamic analyses, refer also to ROAE based on average equity.',
      'ar': 'في ABIX Data Explorer، يُحتسب العائد على حقوق الملكية القياسي بقسمة النتيجة الصافية على حقوق الملكية في 31 ديسمبر. يقيس قدرة البنك على توليد أرباح من أموال المساهمين.'
    },
    'formula': 'ROE = (Résultat Net ÷ Fonds Propres de clôture) × 100',
    'formula_latex': r'\\text{ROE} = \\frac{\\text{Résultat Net}}{\\text{Fonds Propres}_{\\text{clôture}}} \\times 100',
    'interpretation': {
    ...
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Classements', 'Profil Banque', 'Scores ABIX (Rentabilité)', 'Matrice Rentabilité vs Risque'],
    'related_terms': ['return-on-average-equity', 'return-on-assets', 'fonds-propres', 'resultat-net'],
    'display_order': 9
  },
  {
    'id': 'return-on-average-equity',
    'slug': 'return-on-average-equity',
    'term': {'fr': 'Return on Average Equity (ROAE)', 'en': 'Return on Average Equity (ROAE)', 'ar': 'العائد على متوسط حقوق الملكية'},
    'acronym': 'ROAE',
    'aliases': {
      'fr': ['ROAE', 'Rentabilité des fonds propres moyens'],
      'en': ['ROAE', 'Return on Average Equity'],
      'ar': ['العائد على متوسط الأموال الخاصة']
    },
    'category': 'ratios_profitability',
    'short_definition': {
    ...
    'detailed_definition': {
      'fr': 'Le ROAE neutralise les variations significatives de capital intervenues en cours d\'année (augmentations de capital, distributions de dividendes). Il offre une mesure plus fine de la rentabilité économique des capitaux effectivement engagés au fil de l\'exercice.',
      'en': 'ROAE neutralizes intra-year capital variations (equity injections, dividend payouts). It provides a refined economic measurement of returns on capital employed throughout the year.',
      'ar': 'يحيّد ROAE تأثير التغيرات الرأسمالية خلال السنة (زيادة رأس المال، توزيع الأرباح) لتقديم قياس دقيق للمردودية الاقتصادية.'
    },
    'formula': 'ROAE = [Résultat Net ÷ ((Fonds Propres t + Fonds Propres t-1) ÷ 2)] × 100',
    'formula_latex': r'\\text{ROAE} = \\frac{\\text{Résultat Net}}{\\frac{\\text{Fonds Propres}_t + \\text{Fonds Propres}_{t-1}}{2}} \\times 100',
    'interpretation': {
    ...
    'example': {
      'fr': 'Données 2024, Édition ABIX 2025, périmètre constant : Pour une banque affichant 12 Mds DZD de résultat avec 100 Mds DZD de FP en 2023 et 120 Mds DZD en 2024, le ROAE est de 10,9 % (vs 10,0 % de ROE de clôture).',
      'en': '2024 data, ABIX 2025 edition, constant scope: For 12B DZD profit and equity moving from 100B to 120B DZD, ROAE is 10.9% (vs 10.0% closing ROE).',
      'ar': 'بيانات 2024، إصدار ABIX 2025: بنك بربح 12 مليار دج وأموال خاصة ارتفعت من 100 إلى 120 مليار دج، يكون ROAE مساوياً لـ 10.9%.'
    },
    'unit': '%',
    'performance_direction': 'higher_is_better',
    'higher_is_better': True,
    'threshold_type': 'ABIX_BENCHMARK',
    'regulatory_threshold': None,
    'abix_benchmark': 'Standard sectoriel 8-15 %',
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre constant',
    'modules': ['Profil Banque (Analytique)', 'Comparateur Avancé'],
    'related_terms': ['return-on-equity', 'return-on-assets', 'fonds-propres'],
    'display_order': 10
  },
  {
    'id': 'return-on-assets',
    'slug': 'return-on-assets',
    'term': {'fr': 'Return on Assets (Rentabilité des Actifs)', 'en': 'Return on Assets (ROA)', 'ar': 'العائد على الأصول'},
    'acronym': 'ROA',
    'aliases': {
      'fr': ['Rentabilité économique', 'Rendement des actifs', 'ROA de clôture', 'Proxy RoA'],
      'en': ['ROA', 'Return on Total Assets'],
      'ar': ['مردودية الأصول', 'العائد على الموجودات']
    },
    'category': 'ratios_profitability',
    'short_definition': {
    ...
    'formula_latex': r'\\text{ROA} = \\frac{\\text{Résultat Net}}{\\text{Total Bilan}_{\\text{clôture}}} \\times 100',
    'interpretation': {
    ...
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Classements', 'Profil Banque', 'Scores ABIX (Rentabilité)', 'Matrice 4 Quadrants'],
    'related_terms': ['return-on-average-assets', 'return-on-equity', 'total-bilan', 'resultat-net'],
    'display_order': 11
  },
  {
    'id': 'return-on-average-assets',
    'slug': 'return-on-average-assets',
    'term': {'fr': 'Return on Average Assets (ROAA)', 'en': 'Return on Average Assets (ROAA)', 'ar': 'العائد على متوسط الأصول'},
    'acronym': 'ROAA',
    'aliases': {
      'fr': ['ROAA', 'Rentabilité des actifs moyens'],
      'en': ['ROAA', 'Return on Average Assets'],
      'ar': ['العائد على متوسط الموجودات']
    },
    'category': 'ratios_profitability',
    'short_definition': {
    ...
    'detailed_definition': {
      'fr': 'Le ROAA lisse les variations bilancielles intra-annuelles significatives (croissance rapide des dépôts, acquisitions, refinancement). Il constitue le standard académique pour comparer l\'efficience d\'utilisation du bilan dans le temps.',
      'en': 'ROAA smooths out intra-year balance sheet expansions (rapid deposit growth, refinancing). It represents the academic standard for measuring asset efficiency over time.',
      'ar': 'يحيّد ROAA التغيرات الحجمية المفاجئة في الميزانية ويقدم قياساً معيارياً لكفاءة توظيف الأصول عبر الزمن.'
    },
    'formula': 'ROAA = [Résultat Net ÷ ((Total Bilan t + Total Bilan t-1) ÷ 2)] × 100',
    'formula_latex': r'\\text{ROAA} = \\frac{\\text{Résultat Net}}{\\frac{\\text{Total Bilan}_t + \\text{Total Bilan}_{t-1}}{2}} \\times 100',
    'interpretation': {
    ...
    'example': {
      'fr': 'Données 2024, Édition ABIX 2025, périmètre constant : Banque affichant 5 Mds DZD de RN pour un bilan passant de 400 à 500 Mds DZD : ROAA = 1,11 % (vs 1,00 % de ROA de clôture).',
      'en': '2024 data, ABIX 2025 edition, constant scope: 5B DZD profit on assets expanding from 400B to 500B DZD: ROAA is 1.11% (vs 1.00% closing ROA).',
      'ar': 'بيانات 2024، إصدار ABIX 2025: بنك بربح 5 مليار دج وميزانية ارتفعت من 400 إلى 500 مليار دج، يكون ROAA مساوياً لـ 1.11%.'
    },
    'unit': '%',
    'performance_direction': 'higher_is_better',
    'higher_is_better': True,
    'threshold_type': 'ABIX_BENCHMARK',
    'regulatory_threshold': None,
    'abix_benchmark': 'Standard sectoriel 0,8-1,5 %',
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre constant',
    'modules': ['Profil Banque', 'Comparateur Avancé'],
    'related_terms': ['return-on-assets', 'return-on-equity', 'total-bilan'],
    'display_order': 12
  },
  {
    'id': 'coefficient-exploitation',
    'slug': 'coefficient-exploitation',
    'term': {'fr': 'Coefficient d\'Exploitation', 'en': 'Cost-to-Income Ratio (CIR)', 'ar': 'نسبة التكلفة إلى الدخل (معامل الاستغلال)'},
    'acronym': 'CIR',
    'aliases': {
      'fr': ['Cost-to-Income', 'Ratio d\'efficience', 'Charges / PNB'],
      'en': ['Cost-to-Income Ratio', 'CIR', 'Efficiency Ratio', 'Operating Expense Ratio'],
      'ar': ['معامل الاستغلال', 'نسبة الكفاءة التشغيلية', 'المصاريف إلى الدخل']
    },
    'category': 'ratios_profitability',
    'short_definition': {
    ...
    'formula_latex': r'\\text{CIR} = \\frac{\\text{Frais généraux} + \\text{Amortissements}}{\\text{PNB}} \\times 100',
    'interpretation': {
    ...
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Classements', 'Profil Banque', 'Scores ABIX (Efficience)', 'Historique'],
    'related_terms': ['produit-net-bancaire', 'resultat-brut-exploitation', 'conversion-rbe-rn'],
    'display_order': 13
  },
  {
    'id': 'marge-interet-nette',
    'slug': 'marge-interet-nette',
    'term': {'fr': 'Marge d\'Intérêt Nette (NIM)', 'en': 'Net Interest Margin (NIM)', 'ar': 'هامش الفائدة الصافي'},
    'acronym': 'NIM',
    'aliases': {
      'fr': ['NIM', 'Marge nette d\'intermédiation', 'Productivité du bilan'],
      'en': ['Net Interest Margin', 'NIM', 'Interest Spread Margin'],
    ...
    'formula_latex': r'\\text{NIM} = \\frac{\\text{Marge d\'intérêt}}{\\text{Actifs productifs}} \\times 100',
    'interpretation': {
    ...
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Historique'],
    'related_terms': ['produit-net-bancaire', 'total-bilan', 'ratio-prets-depots'],
    'display_order': 14
  },
  {
    'id': 'marge-nette-pnb',
    'slug': 'marge-nette-pnb',
    'term': {'fr': 'Marge Nette sur PNB', 'en': 'Net Profit Margin on NBI', 'ar': 'هامش الربح الصافي على الناتج البنكي'},
    'acronym': 'NPM',
    'aliases': {
      'fr': ['Taux de marge nette', 'Résultat Net / PNB', 'Taux de transformation PNB'],
      'en': ['Net Margin', 'Net Profit Margin', 'Net Income to NBI'],
      'ar': ['هامش الربحية الصافية', 'معدل تحويل الدخل إلى صافي ربح']
    },
    'category': 'ratios_profitability',
    'short_definition': {
    ...
    'formula_latex': r'\\text{Marge Nette} = \\frac{\\text{Résultat Net}}{\\text{PNB}} \\times 100',
    'interpretation': {
    ...
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Scores ABIX (Rentabilité)'],
    'related_terms': ['produit-net-bancaire', 'resultat-net', 'conversion-rbe-rn'],
    'display_order': 15
  },
  {
    'id': 'conversion-rbe-rn',
    'slug': 'conversion-rbe-rn',
    'term': {'fr': 'Taux de Conversion RBE en Résultat Net', 'en': 'GOI to Net Profit Conversion Rate', 'ar': 'معدل تحويل النتيجة التشغيلية إلى نتيجة صافية'},
    'acronym': 'RBE_RN',
    'aliases': {
      'fr': ['Taux de transformation RBE', 'Résultat Net / RBE', 'Efficience de conversion'],
      'en': ['GOI Conversion Rate', 'Operating Profit Retention'],
      'ar': ['معدل الاحتفاظ بالربح التشغيلي', 'كفاءة التحويل الصافي']
    },
    'category': 'ratios_profitability',
    'short_definition': {
      'fr': 'Part du résultat brut d\'exploitation conservée en résultat net après coût du risque et impôts.',
      'en': 'Share of Gross Operating Income retained as net profit after credit risk provisions and taxes.',
      'ar': 'حصة النتيجة الإجمالية للاستغلال المحتفظ بها كربح صافٍ بعد خصم مخصصات المخاطر والضرائب.'
    },
    'detailed_definition': {
      'fr': 'Cet indicateur mesure la résistance de la marge opérationnelle face aux ponctions du coût du risque et de la fiscalité. Un taux de conversion proche de 100 % indique que le résultat opérationnel n\'a pas été amputé par de lourdes dépréciations d\'actifs.',
      'en': 'Measures the resilience of operational income against provisions and taxes. A conversion rate near 100% indicates that gross operating profit was not impaired by credit loss charges.',
      'ar': 'يقيس صمود الأرباح التشغيلية أمام مخصصات الديون المتعثرة والأعباء الضريبية.'
    },
    'formula': 'Taux de Conversion RBE → RN = (Résultat Net ÷ RBE) × 100',
    'formula_latex': r'\\text{Taux de Conversion} = \\frac{\\text{Résultat Net}}{\\text{RBE}} \\times 100',
    'interpretation': {
      'fr': 'Une forte baisse signale un alourdissement brutal du coût du risque ou des pertes exceptionnelles.',
      'en': 'A sharp drop signals an acceleration in credit loss provisioning or non-operating write-offs.',
      'ar': 'الانخفاض الحاد يشير إلى ارتفاع مفاجئ في مخصصات المخاطر أو حدوث خسائر استثنائية.'
    },
    'example': {
    ...
    'threshold_type': 'ABIX_BENCHMARK',
    'regulatory_threshold': None,
    'abix_benchmark': 'Standard sectoriel 65-85 %',
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Scores ABIX (Efficience)'],
    'related_terms': ['resultat-brut-exploitation', 'resultat-net', 'pression-provision-rbe'],
    'display_order': 16
  },
  ...
  # ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    'term': {'fr': 'Ratio de Solvabilité Réglementaire (CAR)', 'en': 'Capital Adequacy Ratio (CAR)', 'ar': 'معدل الملاءة المالية التنظيمية'},
    'acronym': 'CAR',
    'aliases': {
      'fr': ['Ratio Cooke/McDonough', 'Ratio de solvabilité global', 'Fonds propres / Risques pondérés'],
      'en': ['Capital Adequacy Ratio', 'CAR', 'Solvency Ratio', 'Regulatory Capital to RWA'],
      'ar': ['نسبة كفاية رأس المال', 'معدل الملاءة الإجمالي']
    },
    ...
    'formula_latex': r'\\text{CAR} = \\frac{\\text{Fonds Propres Réglementaires}}{\\text{RWA}} \\times 100',
    'interpretation': {
    ...
    'modules': ['Vue Secteur', 'Profil Banque', 'Scores ABIX (Solvabilité)', 'Fiche Prudentielle'],
    'related_terms': ['tier-1-ratio', 'fonds-propres', 'fonds-propres-total-bilan', 'taux-npl'],
    'display_order': 17
  },
  {
    'id': 'tier-1-ratio',
    'slug': 'tier-1-ratio',
    'term': {'fr': 'Ratio de Fonds Propres de Base (Tier 1)', 'en': 'Tier 1 Capital Ratio', 'ar': 'معدل الأموال الخاصة الأساسية (Tier 1)'},
    'acronym': 'Tier1',
    'aliases': {
      'fr': ['Ratio Tier 1', 'Fonds propres durs / RWA', 'Core Tier 1'],
      'en': ['Tier 1 Ratio', 'Core Capital Ratio', 'Going-Concern Capital'],
      'ar': ['نسبة الأموال الخاصة الأساسية', 'معدل الشريحة الأولى']
    },
    'category': 'risk_solvency',
    'short_definition': {
    ...
    'formula_latex': r'\\text{Ratio Tier 1} = \\frac{\\text{Tier 1 Capital}}{\\text{RWA}} \\times 100',
    'interpretation': {
    ...
    'modules': ['Profil Banque', 'Fiche Prudentielle'],
    'related_terms': ['ratio-solvabilite', 'fonds-propres', 'fonds-propres-total-bilan'],
    'display_order': 18
  },
  {
    'id': 'taux-npl',
    'slug': 'taux-npl',
    'term': {'fr': 'Taux de Créances Non Performantes (NPL Ratio)', 'en': 'Non-Performing Loans Ratio (NPL Ratio)', 'ar': 'نسبة القروض المتعثرة (الديون غير المنتجة)'},
    'acronym': 'NPL',
    'aliases': {
      'fr': ['Ratio NPL', 'Taux de créances douteuses', 'Créances compromises / Crédits'],
      'en': ['NPL Ratio', 'Bad Loan Ratio', 'Impaired Loans Ratio', 'Non-Performing Assets'],
      'ar': ['نسبة الديون المتعثرة', 'القروض المشكوك في تحصيلها']
    },
    'category': 'risk_solvency',
    'short_definition': {
    ...
    'detailed_definition': {
      'fr': 'Le taux de NPL (Non-Performing Loans) est l\'indicateur fondamental de qualité des actifs bancaires. Il rapporte le volume des créances douteuses et litigieuses à l\'encours brut total des crédits accordés. Dans le modèle ABIX, c\'est un indicateur à sens inversé (plus faible = plus favorable).',
      'en': 'The NPL ratio is the primary indicator of asset quality. It measures impaired and doubtful loans relative to total gross credit portfolio. In ABIX, it is an inverted metric (lower is better).',
      'ar': 'تعد نسبة الديون المتعثرة المؤشر الجوهري لجودة الأصول البنكية. تقيس حجم الديون المشكوك في استردادها مقارنة بإجمالي القروض الممنوحة. (الأقل = أفضل).'
    },
    'formula': 'Taux NPL = (Créances Non Performantes brutes ÷ Encours Total des Crédits bruts) × 100',
    'formula_latex': r'\\text{Taux NPL} = \\frac{\\text{Créances Douteuses}}{\\text{Total Crédits Bruts}} \\times 100',
    'interpretation': {
      'fr': 'Un taux bas traduit une saine sélection des emprunteurs et un recouvrement efficace. Un taux élevé requiert des provisions additionnelles qui grèvent la rentabilité.',
      'en': 'A low ratio reflects strict underwriting standards and efficient recovery. A high ratio necessitates heavy provisioning that weighs on profit.',
      'ar': 'تدل النسبة المنخفضة على سلامة منح الائتمان وكفاءة التحصيل، بينما تتطلب النسبة المرتفعة مخصصات إضافية تضغط على الربحية.'
    },
    'example': {
      'fr': 'Données sectorielles indicatives : Taux de NPL moyen du secteur bancaire situé autour de 10-12 %, avec une disparité marquée entre les banques publiques (portefeuilles historiques) et les filiales privées (souvent < 5 %).',
      'en': 'Indicative sector data: Average sector NPL ratio around 10-12%, with marked divergence between public banks (legacy exposures) and private subsidiaries (often < 5%).',
      'ar': 'بيانات قطاعية استرشادية: متوسط نسبة القروض المتعثرة يقارب 10-12%، مع تفاوت بين البنوك العمومية (محافظ تاريخية) والبنوك الخاصة (أقل من 5% عادة).'
    },
    'unit': '%',
    'performance_direction': 'lower_is_better',
    'higher_is_better': False,
    ...
    'regulatory_threshold': None,
    'abix_benchmark': 'Qualité excellente < 4 %, Modérée 4-8 %, Vigilance > 10 %',
    'example_vintage': 'Données sectorielles indicatives, Édition ABIX 2025',
    'modules': ['Profil Banque', 'Comparateur', 'Matrice Rentabilité vs Risque'],
    'related_terms': ['taux-couverture-npl', 'effort-provisionnement', 'credits-clientele'],
    'display_order': 19
  },
  {
    'id': 'taux-couverture-npl',
    'slug': 'taux-couverture-npl',
    'term': {'fr': 'Taux de Couverture des NPL', 'en': 'NPL Coverage Ratio', 'ar': 'نسبة تغطية القروض المتعثرة بالمخصصات'},
    'acronym': 'COV',
    'aliases': {
      'fr': ['Coverage Ratio', 'Couverture des créances douteuses', 'Provisions / NPL'],
      'en': ['NPL Coverage Ratio', 'Provisioning Coverage', 'Impairment Coverage'],
      'ar': ['نسبة تغطية الديون المتعثرة', 'معدل التغطية بالمخصصات']
    },
    'category': 'risk_solvency',
    'short_definition': {
    ...
    'detailed_definition': {
      'fr': 'Le taux de couverture évalue le degré d\'anticipation et de protection financière de la banque face aux pertes prévisibles sur son portefeuille de créances dégradées. Un taux élevé signifie que la majeure partie des créances compromises est déjà couverte par des réserves au bilan.',
      'en': 'Assesses the degree of balance sheet protection against anticipated credit losses on impaired loans. A high coverage ratio means that the bulk of bad debt is already provisioned.',
      'ar': 'يقيم درجة الحماية المالية للبنك أمام الخسائر المحتملة للديون المتعثرة. وتعني النسبة المرتفعة أن معظم الديون المشكوك فيها مغطاة بمخصصات مسجلة مسبقاً.'
    },
    'formula': 'Taux de Couverture = (Stock de Provisions pour Dépréciation ÷ Encours Brut des NPL) × 100',
    'formula_latex': r'\\text{Taux de Couverture} = \\frac{\\text{Stock de Provisions}}{\\text{Créances NPL}} \\times 100',
    'interpretation': {
      'fr': 'Un taux supérieur à 70-80 % est considéré comme un standard de confort prudentiel sur les marchés bancaires émergents.',
      'en': 'A coverage ratio above 70-80% is widely regarded as a prudential comfort benchmark in emerging banking markets.',
      'ar': 'تعتبر نسبة التغطية التي تتجاوز 70-80% معياراً مريحاً من حيث الحيطة والحذر في الأسواق المصرفية.'
    },
    'example': {
    ...
    'threshold_type': 'MARKET_REFERENCE',
    'regulatory_threshold': None,
    'abix_benchmark': 'Couverture prudente recommandée >= 70-80 %',
    'example_vintage': 'Exemple méthodologique calibré',
    'modules': ['Profil Banque', 'Comparateur', 'Fiche Prudentielle'],
    'related_terms': ['taux-npl', 'effort-provisionnement', 'credits-clientele'],
    'display_order': 20
  },
  {
    'id': 'effort-provisionnement',
    'slug': 'effort-provisionnement',
    'term': {'fr': 'Effort de Provisionnement / Prêts (Coût du Risque Proxy)', 'en': 'Provisioning Effort to Loans (Cost of Risk Proxy)', 'ar': 'جهد المخصصات إلى القروض (بديل تكلفة المخاطر)'},
    'acronym': 'PROV_CRE',
    'aliases': {
      'fr': ['Provisions nettes / Crédits', 'Coût du risque relatif', 'Effort de couverture annuel'],
      'en': ['Net Provisions to Loans', 'Cost of Risk to Lending', 'Provisioning Rate'],
      'ar': ['المخصصات الصافية إلى القروض', 'معدل تجنيب المخصصات']
    },
    'category': 'risk_solvency',
    'short_definition': {
    ...
    'detailed_definition': {
      'fr': 'Cet indicateur constitue le proxy direct du coût du risque dans ABIX Data Explorer. Il mesure l\'impact annuel du provisionnement des risques de crédit sur le portefeuille d\'engagements. Note méthodologique : Cet indicateur rapporte le flux disponible dans les données ABIX aux prêts ; il ne se substitue pas au ratio prudentiel de couverture globale des NPL.',
      'en': 'Serves as the direct proxy for cost of risk in ABIX Data Explorer. It measures the annual P&L charge required to absorb credit deterioration across the loan book.',
      'ar': 'يمثل المؤشر البديل لتكلفة المخاطر في ABIX Data Explorer، ويقيس العبء السنوي للمخصصات المجنبة مقارنة بحجم القروض.'
    },
    'formula': 'Effort de Provisionnement = (Dotations Nettes aux Provisions ÷ Encours des Prêts) × 100',
    'formula_latex': r'\\text{Effort Provisions} = \\frac{\\text{Dotations Nettes aux Provisions}}{\\text{Crédits Clientèle}} \\times 100',
    'interpretation': {
      'fr': 'Un effort de provisionnement faible et stable traduit une excellente maîtrise de la sinistralité des emprunteurs.',
      'en': 'A low and steady provisioning effort indicates disciplined underwriting and low default incidence.',
      'ar': 'يدل انخفاض واستقرار هذا المعدل على جودة المحفظة الائتمانية والتحكم في حالات التعثر.'
    },
    'example': {
    ...
    'threshold_type': 'ABIX_BENCHMARK',
    'regulatory_threshold': None,
    'abix_benchmark': 'Faible < 0,5 %, Moyen 0,5-1,2 %, Élevé > 1,5 %',
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Scores ABIX (Risque)', 'Historique'],
    'related_terms': ['taux-npl', 'pression-provision-rbe', 'credits-clientele'],
    'display_order': 21
  },
  {
    'id': 'pression-provision-rbe',
    'slug': 'pression-provision-rbe',
    'term': {'fr': 'Pression du Provisionnement sur le RBE', 'en': 'Provision Burden on Gross Operating Income', 'ar': 'ضغط المخصصات على الناتج التشغيلي'},
    'acronym': 'PROV_RBE',
    'aliases': {
      'fr': ['Provisions / RBE', 'Pression du risque d\'exploitation'],
      'en': ['Provisions to GOI', 'Operating Profit Absorption by Risk'],
      'ar': ['المخصصات إلى الربح التشغيلي']
    },
    'category': 'risk_solvency',
    'short_definition': {
    ...
    'detailed_definition': {
      'fr': 'Mesure dans quelle proportion le résultat opérationnel de la banque est amputé par le coût du risque avant impôts. Note méthodologique ABIX : Une hausse de ce ratio peut refléter soit une dégradation du risque client, soit une politique délibérée de renforcement prudentiel des réserves.',
      'en': 'Measures the proportion of operating income absorbed by credit loss provisioning. ABIX note: An increase may reflect deteriorating borrower quality or a proactive, conservative provisioning stance.',
      'ar': 'يقيس نسبة الأرباح التشغيلية التي يتم تجنيبها كمخصصات للمخاطر. ملاحظة ABIX: قد يعكس ارتفاع النسبة تراجع جودة الائتمان أو سياسة حيطية استباقية لتعزيز الاحتياطيات.'
    },
    'formula': 'Pression Provisions / RBE = (Dotations Nettes aux Provisions ÷ RBE) × 100',
    'formula_latex': r'\\text{Pression Provisions} = \\frac{\\text{Dotations Nettes}}{\\text{RBE}} \\times 100',
    'interpretation': {
      'fr': 'Un ratio inférieur à 20-25 % permet de préserver l\'essentiel de la marge opérationnelle pour le résultat net final.',
      'en': 'A ratio below 20-25% preserves the bulk of operating surplus for net bottom-line profit.',
      'ar': 'النسبة الأقل من 20-25% تحافظ على معظم الفائض التشغيلي ليتحول إلى أرباح صافية.'
    },
    'example': {
    ...
    'threshold_type': 'ABIX_BENCHMARK',
    'regulatory_threshold': None,
    'abix_benchmark': 'Confortable < 20 %, Modéré 20-35 %, Pression forte > 40 %',
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Scores ABIX (Efficience & Risque)'],
    'related_terms': ['resultat-brut-exploitation', 'conversion-rbe-rn', 'effort-provisionnement'],
    'display_order': 22
  },
  {
    'id': 'fonds-propres-total-bilan',
    'slug': 'fonds-propres-total-bilan',
    'term': {'fr': 'Fonds Propres / Total Bilan (Capitalisation Comptable)', 'en': 'Equity to Total Assets (Accounting Capitalization)', 'ar': 'حقوق الملكية إلى إجمالي الميزانية (الرسملة المحاسبية)'},
    'acronym': 'FP_TB',
    'aliases': {
      'fr': ['Taux de capitalisation bilancielle', 'Capitaux propres / Bilan', 'Autonomie financière'],
      'en': ['Equity-to-Assets Ratio', 'Capitalization Ratio', 'Equity Cushion'],
      'ar': ['نسبة الرسملة المحاسبية', 'الأموال الخاصة إلى الميزانية']
    },
    'category': 'risk_solvency',
    'short_definition': {
    ...
    'detailed_definition': {
      'fr': 'Indicateur comptable direct d\'assise financière et d\'indépendance. Il mesure le pourcentage des actifs financé par des capitaux permanents sans exigibilité de remboursement. Note méthodologique ABIX : Cet indicateur constitue un ratio de capitalisation comptable simplifié et ne remplace pas les ratios réglementaires de solvabilité fondés sur la pondération des risques (RWA).',
      'en': 'Direct accounting metric of balance sheet equity backing. ABIX note: This is a simplified accounting capitalization ratio and does not substitute for regulatory solvency ratios based on risk-weighted assets (RWA).',
      'ar': 'مؤشر محاسبي للاستقلالية المالية. ملاحظة ABIX: يعد هذا المؤشر نسبة رسملة محاسبية مبسطة ولا يعوض نسب الملاءة التنظيمية المعتمدة على ترجيح المخاطر.'
    },
    'formula': 'Fonds Propres / Total Bilan = (Fonds Propres Comptables ÷ Total Bilan) × 100',
    'formula_latex': r'\\text{FP / Bilan} = \\frac{\\text{Fonds Propres}}{\\text{Total Bilan}} \\times 100',
    'interpretation': {
      'fr': 'Un ratio élevé (> 12-15 %) traduit une capitalisation solide et une grande sécurité financière.',
      'en': 'A high ratio (> 12-15%) indicates strong capital backing and high financial safety.',
      'ar': 'تدل النسبة المرتفعة (أكثر من 12-15%) على متانة مالية وأمان عالٍ للمودعين.'
    },
    'example': {
      'fr': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques : Ratio sectoriel moyen de 13,2 %, reflétant une solide capitalisation de la place bancaire algérienne.',
    ...
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Scores ABIX (Capitalisation)', 'Classements'],
  ...
  },
  {
    ...
    'term': {'fr': 'Levier Comptable (Total Bilan / Fonds Propres)', 'en': 'Accounting Leverage (Assets / Equity)', 'ar': 'الرافعة المالية المحاسبية (الميزانية / الأموال الخاصة)'},
    'acronym': 'LEV',
    'aliases': {
      'fr': ['Levier bilanciel', 'Multiple de fonds propres', 'Actifs / Capitaux propres'],
      'en': ['Accounting Leverage', 'Equity Multiplier', 'Assets to Equity'],
      'ar': ['مضاعف حقوق الملكية', 'الرافعة المحاسبية']
    },
    'category': 'risk_solvency',
    'short_definition': {
    ...
    'formula_latex': r'\\text{Levier} = \\frac{\\text{Total Bilan}}{\\text{Fonds Propres}}',
    'interpretation': {
    ...
    'related_terms': ['fonds-propres-total-bilan', 'fonds-propres', 'total-bilan'],
    'display_order': 24
  },
  ...
  # ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    'term': {'fr': 'Ratio Prêts / Dépôts (LDR)', 'en': 'Loan-to-Deposit Ratio (LDR)', 'ar': 'نسبة القروض إلى الودائع (معدل التحويل)'},
    'acronym': 'LDR',
    'aliases': {
      'fr': ['LDR', 'Taux de transformation des dépôts', 'Crédits / Dépôts'],
      'en': ['Loan-to-Deposit Ratio', 'LDR', 'Transformation Ratio', 'LTD Ratio'],
      'ar': ['نسبة القروض إلى الودائع', 'معدل التحويل المصرفي']
    },
    ...
    'formula_latex': r'\\text{LDR} = \\frac{\\text{Crédits Clientèle}}{\\text{Dépôts Clientèle}} \\times 100',
    'interpretation': {
    ...
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Comparateur', 'Historique'],
    'related_terms': ['depots-clientele', 'credits-clientele', 'ratio-lcr'],
    'display_order': 25
  },
  {
    'id': 'prets-total-actif',
    'slug': 'prets-total-actif',
    'term': {'fr': 'Part des Prêts dans l\'Actif (Crédits / Bilan)', 'en': 'Loans to Total Assets', 'ar': 'حصة القروض في الأصول (القروض / الميزانية)'},
    'acronym': 'CRE_TB',
    'aliases': {
      'fr': ['Crédits / Bilan', 'Orientation crédit du bilan', 'Taux d\'emploi en crédits'],
      'en': ['Loans to Assets', 'Loan Asset Share', 'Credit Intensity'],
      'ar': ['نسبة القروض إلى الأصول', 'كثافة الائتمان في الميزانية']
    },
    'category': 'liquidity_intermediation',
    'short_definition': {
    ...
    'formula_latex': r'\\text{Prêts / Bilan} = \\frac{\\text{Crédits Clientèle}}{\\text{Total Bilan}} \\times 100',
    'interpretation': {
    ...
    'abix_benchmark': None,
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Allocation d\'actifs', 'Comparateur'],
    'related_terms': ['credits-clientele', 'total-bilan', 'bdt-total-actif', 'ratio-prets-depots'],
    'display_order': 26
  },
  {
    'id': 'bdt-total-actif',
    'slug': 'bdt-total-actif',
    'term': {'fr': 'Part des Bons du Trésor dans l\'Actif (BDT / Bilan)', 'en': 'Treasury Securities to Total Assets', 'ar': 'حصة سندات الخزينة في الأصول (السندات / الميزانية)'},
    'acronym': 'BDT_TB',
    'aliases': {
      'fr': ['BDT / Bilan', 'Exposition souveraine bilancielle', 'Titres d\'État / Actif'],
      'en': ['Treasury to Assets', 'Sovereign Debt Exposure', 'Government Securities Share'],
      'ar': ['نسبة السندات إلى الأصول', 'التعرض للسندات السيادية']
    },
    'category': 'liquidity_intermediation',
    'short_definition': {
    ...
    'formula_latex': r'\\text{BDT / Bilan} = \\frac{\\text{Bons du Trésor}}{\\text{Total Bilan}} \\times 100',
    'interpretation': {
    ...
    'abix_benchmark': None,
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Allocation d\'actifs'],
    'related_terms': ['bons-du-tresor', 'total-bilan', 'prets-total-actif'],
    'display_order': 27
  },
  {
    'id': 'depots-total-passif',
    'slug': 'depots-total-passif',
    'term': {'fr': 'Part des Dépôts dans le Passif (Dépôts / Bilan)', 'en': 'Deposits to Total Assets (Liabilities)', 'ar': 'حصة الودائع في الخصوم (الودائع / الميزانية)'},
    'acronym': 'DEP_TB',
    'aliases': {
      'fr': ['Dépôts / Bilan', 'Taux d\'adossement dépôts', 'Ressources clientèle / Passif'],
      'en': ['Deposits to Assets', 'Customer Funding Ratio', 'Deposit Funding Share'],
      'ar': ['نسبة الودائع إلى الخصوم', 'هيكل التمويل بالودائع']
    },
    'category': 'liquidity_intermediation',
    'short_definition': {
    ...
    'formula_latex': r'\\text{Dépôts / Bilan} = \\frac{\\text{Dépôts Clientèle}}{\\text{Total Bilan}} \\times 100',
    'interpretation': {
    ...
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Historique'],
    'related_terms': ['depots-clientele', 'total-bilan', 'ratio-prets-depots'],
    'display_order': 28
  },
  {
    'id': 'autres-actifs',
    'slug': 'autres-actifs',
    'term': {'fr': 'Autres Actifs (Solde Résiduel de l\'Actif)', 'en': 'Other Assets (Residual Balance)', 'ar': 'الأصول الأخرى (الرصيد المتبقي للأصول)'},
    'acronym': 'AA',
    'aliases': {
      'fr': ['Solde résiduel de l\'actif', 'Autres emplois bilanciels'],
      'en': ['Other Assets', 'Residual Assets'],
      'ar': ['الموجودات الأخرى', 'الأصول المتبقية']
    },
    'category': 'liquidity_intermediation',
    'short_definition': {
    ...
    'formula_latex': r'\\text{Autres Actifs} = \\text{Total Bilan} - \\text{Prêts} - \\text{BDT}',
    'interpretation': {
    ...
    'example_vintage': 'Données 2024, Édition ABIX 2025, périmètre de 20 banques',
    'modules': ['Vue Secteur', 'Profil Banque', 'Allocation d\'actifs'],
    'related_terms': ['total-bilan', 'credits-clientele', 'bons-du-tresor'],
    'display_order': 29
  },
  {
    'id': 'ratio-lcr',
    'slug': 'ratio-lcr',
    'term': {'fr': 'Ratio de Liquidité Court Terme (LCR)', 'en': 'Liquidity Coverage Ratio (LCR)', 'ar': 'نسبة تغطية السيولة قصيرة الأجل (LCR)'},
    'acronym': 'LCR',
    'aliases': {
      'fr': ['LCR', 'Ratio de liquidité à 30 jours', 'Liquidité Bâle III'],
      'en': ['Liquidity Coverage Ratio', 'LCR', 'Short-Term Liquidity Ratio'],
      'ar': ['نسبة تغطية السيولة', 'معيار السيولة لـ 30 يوماً']
    },
    'category': 'liquidity_intermediation',
    'short_definition': {
    ...
    'formula_latex': r'\\text{LCR} = \\frac{\\text{HQLA}}{\\text{Sorties Nettes 30j}} \\times 100',
    'interpretation': {
    ...
    'modules': ['Fiche Prudentielle', 'Comparateur'],
    'related_terms': ['ratio-nsfr', 'ratio-prets-depots', 'depots-clientele'],
    'display_order': 30
  },
  {
    'id': 'ratio-nsfr',
    'slug': 'ratio-nsfr',
    'term': {'fr': 'Ratio de Financement Stable Net (NSFR)', 'en': 'Net Stable Funding Ratio (NSFR)', 'ar': 'نسبة التمويل المستقر الصافي (NSFR)'},
    'acronym': 'NSFR',
    'aliases': {
      'fr': ['NSFR', 'Ratio de liquidité structurelle', 'Financement stable à 1 an'],
      'en': ['Net Stable Funding Ratio', 'NSFR', 'Structural Liquidity Ratio'],
      'ar': ['نسبة التمويل المستقر', 'معيار السيولة الهيكلية']
    },
    'category': 'liquidity_intermediation',
    'short_definition': {
    ...
    'formula_latex': r'\\text{NSFR} = \\frac{\\text{ASF}}{\\text{RSF}} \\times 100',
    'interpretation': {
    ...
    'example_vintage': 'Exemple méthodologique normatif Bâle III',
    'modules': ['Fiche Prudentielle', 'Comparateur'],
    'related_terms': ['ratio-lcr', 'ratio-prets-depots', 'fonds-propres'],
    'display_order': 31
  },
  ...
  },
  {
    ...
    'formula_latex': r'\\text{Médiane} = \\begin{cases} X_{\\frac{N+1}{2}} & \\text{si } N \\text{ impair} \\\\ \\frac{X_{\\frac{N}{2}} + X_{\\frac{N}{2}+1}}{2} & \\text{si } N \\text{ pair} \\end{cases}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'Q_1 = P_{25},\\quad Q_2 = P_{50} = \\text{Médiane},\\quad Q_3 = P_{75}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\sigma = \\sqrt{\\frac{1}{N}\\sum_{i=1}^N (X_i - \\bar{X})^2}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{CV} = \\frac{\\sigma}{\\bar{X}} \\times 100',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{CAGR} = \\left[ \\left(\\frac{V_{\\text{final}}}{V_{\\text{initial}}}\\right)^{\\frac{1}{n}} - 1 \\right] \\times 100',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{YoY} = \\frac{V_t - V_{t-1}}{V_{t-1}} \\times 100 \\quad | \\quad \\Delta = V_t - V_{t-1} \\text{ (pts)}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{Indice}_t = \\frac{V_t}{V_{\\text{base}}} \\times 100',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'r = \\frac{\\sum_{i=1}^N (X_i - \\bar{X})(Y_i - \\bar{Y})}{\\sqrt{\\sum_{i=1}^N (X_i - \\bar{X})^2 \\sum_{i=1}^N (Y_i - \\bar{Y})^2}}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'Z = \\frac{X - \\bar{X}}{\\sigma} \\quad | \\quad Z_{\\text{robuste}} = 0{,}6745 \\times \\frac{X - \\text{Médiane}}{\\text{MAD}}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{Score} = \\begin{cases} \\frac{X - \\text{Min}}{\\text{Max} - \\text{Min}} \\times 100 & \\text{si élevé favorable} \\\\ \\frac{\\text{Max} - X}{\\text{Max} - \\text{Min}} \\times 100 & \\text{si faible favorable} \\end{cases}',
    'interpretation': {
  ...
  # ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    'formula_latex': r'\\text{Part de Marché}_i = \\frac{V_i}{\\sum_{k=1}^N V_k} \\times 100',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\Delta \\text{Part de Marché} = \\text{Part}_t - \\text{Part}_{t-1} \\quad \\text{(pts)}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{Total} = \\sum \\text{Banques Publiques} + \\sum \\text{Banques Privées}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{Périmètre Constant} = \\{ i \\in \\text{Banques} \\mid \\forall t \\in [t_0, t_n], V_{i,t} \\neq \\emptyset \\}',
    'interpretation': {
  ...
  # ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    'formula_latex': r'\\text{HHI} = \\sum_{i=1}^N (s_i)^2 \\quad \\text{avec } s_i \\text{ en \\%}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{CR}_k = \\sum_{i=1}^k \\text{Part}_{(i)} \\quad (k = 3, 5)',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'N_{\\text{eff}} = \\frac{10\\,000}{\\text{HHI}}',
    'interpretation': {
  ...
  # ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    'formula_latex': r'\\text{Profil 4D} = \\{ \\text{Score}_{\\text{Rentabilité}}, \\text{Score}_{\\text{Croissance}}, \\text{Score}_{\\text{Efficience}}, \\text{Score}_{\\text{Capitalisation}} \\}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{Alertes} = \\{ f(X_t, X_{t-1}, \\text{Benchmark}) \\mid \\text{Conditions Arithmétiques Déterministes v1.0} \\}',
    'interpretation': {
  ...
  },
  {
    ...
    'formula_latex': r'\\text{Score E-Banking} = \\sum_{k=1}^m w_k \\times S_k \\quad \\text{avec } \\sum w_k = 1',
    'interpretation': {
